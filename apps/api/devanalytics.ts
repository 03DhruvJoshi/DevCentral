...
router.get("/api/analytics/cicd/:owner/:repo", async (req, res) => {
  try {
    const { owner, repo } = req.params;
    ...
    const avgQueue = queueVsExec.length
      ? +(
          queueVsExec.reduce((s, r) => s + r!.queue_min, 0) / queueVsExec.length
        ).toFixed(1)
      : null;
    const avgExec = queueVsExec.length
      ? +(
          queueVsExec.reduce((s, r) => s + r!.exec_min, 0) / queueVsExec.length
        ).toFixed(1)
      : null;
